...
#define CAMPAIGN_ONE_BROGGUT_GOAL 2500 // 2500
#define CAMPAIGN_ONE_TIME_LIMIT 4.0f
...
- (id)initWithLoaded:(BOOL)loaded {
    ...
    if (self) {
        [startObject setMissionTextTwo:[NSString stringWithFormat:@"- Collect %i Brogguts in %i minutes", CAMPAIGN_ONE_BROGGUT_GOAL, (int)CAMPAIGN_ONE_TIME_LIMIT]];
        ...
        if (!loaded) {
            ...
            [dia1 setDialogueText:@"We've put you on a new colony, where we need you to collect a large number of brogguts in a limited amount of time. You are provided with enough blocks so you don't need to worry about the craft limit, so hurry and mine some brogguts!"];
            [sceneDialogues addObject:dia1];
            [dia1 release];
            
            DialogueObject* dia2 = [[DialogueObject alloc] init];
            [dia2 setDialogueActivateTime:CAMPAIGN_ONE_TIME_LIMIT * 60.0f / 2];
            ...
            [spawner pauseSpawnerForDuration:(CAMPAIGN_ONE_TIME_LIMIT * 60.0f) + 1.0f];
